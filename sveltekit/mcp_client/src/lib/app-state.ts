...
export class AppState {
  ...
  async handlePPImageClick(event: MouseEvent) {
    ...
    try {
      ...
      if (!this.ppPickPoint) {
        this.ppPickPoint = pt;
        // // ピック対象が検出オブジェクトの場合、その直径から把持幅を自動設定
        // if (hoveredObject && hoveredObject.ground_center?.r) {
        //     const diameter = hoveredObject.ground_center.r * 2;
        //     // 直径より少し小さい値で把持（例: 5mm）。0-25mmの範囲にクランプ。
        //     const targetWidth = Math.max(0, Math.min(MAX_GRIP_WIDTH, diameter - 5));
        //     this.ppGripWidth = Math.round(targetWidth);
        // }
        ...
        this.ppPickPoint = pt;
        this.ppPlacePoint = null;
        // // ピック対象が検出オブジェクトの場合、同様に把持幅を再設定
        // if (hoveredObject && hoveredObject.ground_center?.r) {
        //     const diameter = hoveredObject.ground_center.r * 2;
        //     const targetWidth = Math.max(0, Math.min(MAX_GRIP_WIDTH, diameter - 5));
        //     this.ppGripWidth = Math.round(targetWidth);
        // } else {
        //     this.ppGripWidth = 0; // オブジェクト上でなければリセット
        // }
      }
    } catch (e: any) {
